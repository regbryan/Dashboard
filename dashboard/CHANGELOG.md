...
- **Per-request correlation ID** via `x-request-id` header + AsyncLocalStorage, now wired on **every** route handler (40 routes across `app/api/**` + `app/auth/callback`). Middleware seeds the ID from `x-vercel-id` (or generates a UUID for local dev), sets it on the rewritten request headers and the response. Routes wrapped in `withRequestContext()` (or `withRequestContextFromHeaders()` for no-arg handlers) propagate the ID through ALS — every `logger.*` call inside auto-tags entries. 6 tests in `tests/request-context.spec.ts` cover the ALS round-trip + concurrent isolation. See [docs/observability.md](docs/observability.md) for the pattern.
